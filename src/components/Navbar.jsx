...
const Navbar = () => {
  ...
  return (
    <div className="flex px-4 justify-between items-center w-full h-20 bg-black text-white fixed">
      <div>
        <h1 className="text-5xl ml-2">Tanya singh</h1>
      </div>
      <ul className="hidden md:flex">
        ...
        <NavLink to={'/technicalstack'} className="px-4 cursor-pointer capitalize font-medium text-gray-500 hover:scale-105 duration-200">
          Technical Stack
        </NavLink>
      ...
      {showNav && (
        <ul className="flex flex-col justify-center h-screen w-full absolute top-0 left-0 items-center bg-gradient-to-b from-black to-gray-800 text-gray-500">
          ...
          <NavLink to={'/technicalstack'} onClick={() => setShowNav(!showNav)} className="px-4 cursor-pointer py-6 capitalize text-4xl">
            TechnicalStack
          </NavLink>
